...
SOURCE_ACCOUNT = get_guid('Assets:JPY - Current Assets:Reward Programs:dPoint')
...
def parse_transactions(raw_data):
    ...
    for line in raw_data.strip().split('\n'):
        ...
        transactions.append({'date': date, 'desc': desc, 'points': points})
...
def get_transaction_info(idx, tx):
    ...
    desc = tx['desc']
    # Expiry
    if desc == 'Point Expiry':
        return get_guid('Expenses:Point Lapse'), 'Point Expiry'
    # Earn
    if tx['points'] >= 0:
        return get_guid('Income:Point Charge'), desc
    # Use: known merchants
    if 'Mos Burger' in desc:
        return get_guid('Expenses:Foods:Dining'), desc
    return None, desc


def output_review(transactions):
    print(f"{'ID':<4} {'Date':<14} {'Desc':<40} {'Transfer':<45} {'Increase':>10} {'Decrease':>10}")
    print("-" * 125)
    ...
    for idx, tx in enumerate(transactions, 1):
        ...
        print(f"{idx:<4} {date_display:<14} {description or '':<40} {transfer:<45} {increase:>10} {decrease:>10}")
