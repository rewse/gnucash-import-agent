#!/usr/bin/env python3
"""ANA Super Flyers Gold Card Statement Importer
...
3. Run: python3 tmp/ana_super_flyers_gold_card_import_YYYYMMDD.py review
4. Run: python3 tmp/ana_super_flyers_gold_card_import_YYYYMMDD.py sql
"""
import json
import re
import sys
...
SOURCE_ACCOUNT = get_guid('Liabilities:Credit Card:ANA Super Flyers Gold Card')
...
def parse_transactions(raw_data):
    ...
    for line in raw_data.strip().split('\n'):
        ...
        if not line or 'ご利用分' in line or 'お支払金額総合計' in line:
            continue
        # Strip B#/# markers before date matching
        stripped = re.sub(r'^[B]?#\t?', '', line)
        ...
        if re.match(r'[\d,]+$', parts[2].strip()):
            ...
            elif len(parts) > 6 and parts[6].strip() and parts[6].strip() != '◎':
...
def main():
    if len(sys.argv) < 2 or sys.argv[1] not in ('review', 'sql'):
        print('Usage: python3 ana_super_flyers_gold_card_import.py [review|sql]', file=sys.stderr)
        sys.exit(1)
