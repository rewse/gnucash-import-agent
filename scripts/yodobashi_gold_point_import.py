#!/usr/bin/env python3
"""
Yodobashi Gold Point Statement Importer

Usage:
1. Copy raw data into RAW_DATA (tab-separated: date, type, description, points, account)
2. Run: python3 scripts/yodobashi_gold_point_import.py review    # Review transactions
3. Run: python3 scripts/yodobashi_gold_point_import.py sql       # Generate SQL
"""
import json
import uuid
import sys
...
SOURCE_ACCOUNT = get_guid('Assets:JPY - Current Assets:Reward Programs:Yodobashi Gold Point')
...
def output_review(transactions):
    print(f"{'ID':<4} {'Date':<14} {'Type':<5} {'Desc':<30} {'Transfer':<45} {'Increase':>10} {'Decrease':>10}")
    print("-" * 120)
    ...
    for idx, tx in enumerate(transactions, 1):
        ...
        print(f"{idx:<4} {date_display:<14} {tx['type']:<5} {tx['desc']:<30} {tx['account']:<45} {increase:>10} {decrease:>10}")
...
def output_sql(transactions):
    ...
    for idx, tx in enumerate(transactions, 1):
        ...
        print(f"VALUES ('{split1_guid}', '{tx_guid}', '{SOURCE_ACCOUNT}', '', '', 'c', NULL, {tx['points']}, 1, {tx['points']}, 1, NULL);")
        print(f"INSERT INTO splits (guid, tx_guid, account_guid, memo, action, reconcile_state, reconcile_date, value_num, value_denom, quantity_num, quantity_denom, lot_guid)")
        print(f"VALUES ('{split2_guid}', '{tx_guid}', '{account_guid}', '', '', 'c', NULL, {-tx['points']}, 1, {-tx['points']}, 1, NULL);")
        print()
...
def main():
    if len(sys.argv) < 2 or sys.argv[1] not in ['review', 'sql']:
        print("Usage: python3 yodobashi_gold_point_import.py [review|sql]", file=sys.stderr)
        sys.exit(1)
