...
3. Run: python3 scripts/lumine_card_import.py review    # Review transactions
4. Run: python3 scripts/lumine_card_import.py sql       # Generate SQL
"""
import csv
import io
import json
...
def output_sql(transactions):
    ...
    for idx, tx in enumerate(transactions, 1):
        ...
        print(f"INSERT INTO splits (guid, tx_guid, account_guid, memo, action, reconcile_state, reconcile_date, value_num, value_denom, quantity_num, quantity_denom, lot_guid)")
        print(f"VALUES ('{split2_guid}', '{tx_guid}', '{account}', '', '', 'c', NULL, {-value_num}, 1, {-value_num}, 1, NULL);")
        print()
