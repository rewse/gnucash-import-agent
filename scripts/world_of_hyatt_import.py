#!/usr/bin/env python3
"""
World of Hyatt Statement Importer

Usage:
1. Copy raw data into RAW_DATA (tab-separated: date, description, amount, account)
2. Run: python3 scripts/world_of_hyatt_import.py review    # Review transactions
3. Run: python3 scripts/world_of_hyatt_import.py sql       # Generate SQL
"""
import json
import uuid
import sys
...
SOURCE_ACCOUNT = get_guid('Assets:JPY - Current Assets:Reward Programs:World of Hyatt')
...
def points_to_jpy(points):
    """Convert points to JPY (1 pt = 0.4 JPY)."""
...
def output_review(transactions):
    ...
    for idx, tx in enumerate(transactions, 1):
        ...
        jpy = points_to_jpy(abs(tx['amount']))
        increase = f"{tx['amount']:,} pt (¥{jpy:,})" if tx['amount'] > 0 else ""
        decrease = f"{abs(tx['amount']):,} pt (¥{jpy:,})" if tx['amount'] < 0 else ""
...
def output_sql(transactions):
    ...
    for idx, tx in enumerate(transactions, 1):
        ...
        print(f"VALUES ('{split1_guid}', '{tx_guid}', '{SOURCE_ACCOUNT}', '', '', 'c', NULL, {jpy_value}, 1, {jpy_value}, 1, NULL);")
        print(f"INSERT INTO splits (guid, tx_guid, account_guid, memo, action, reconcile_state, reconcile_date, value_num, value_denom, quantity_num, quantity_denom, lot_guid)")
        print(f"VALUES ('{split2_guid}', '{tx_guid}', '{account_guid}', '', '', 'c', NULL, {-jpy_value}, 1, {-jpy_value}, 1, NULL);")
        print()
...
def main():
    if len(sys.argv) < 2 or sys.argv[1] not in ['review', 'sql']:
        print("Usage: python3 world_of_hyatt_import.py [review|sql]", file=sys.stderr)
        sys.exit(1)
