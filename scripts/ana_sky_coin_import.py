#!/usr/bin/env python3
"""
ANA SKY Coin Statement Importer

Usage:
1. Copy raw data into RAW_DATA (tab-separated: date, description, amount, account)
2. Run: python3 scripts/ana_sky_coin_import.py review    # Review transactions
3. Run: python3 scripts/ana_sky_coin_import.py sql       # Generate SQL
"""
import json
import uuid
import sys
...
SOURCE_ACCOUNT = get_guid('Assets:JPY - Current Assets:Reward Programs:ANA SKY Coin')
...
def output_sql(transactions):
    ...
    for idx, tx in enumerate(transactions, 1):
        ...
        print(f"VALUES ('{split1_guid}', '{tx_guid}', '{SOURCE_ACCOUNT}', '', '', 'c', NULL, {tx['amount']}, 1, {tx['amount']}, 1, NULL);")
        print(f"INSERT INTO splits (guid, tx_guid, account_guid, memo, action, reconcile_state, reconcile_date, value_num, value_denom, quantity_num, quantity_denom, lot_guid)")
...
def main():
    if len(sys.argv) < 2 or sys.argv[1] not in ['review', 'sql']:
        print("Usage: python3 ana_sky_coin_import.py [review|sql]", file=sys.stderr)
        sys.exit(1)
